...
econ_situation_nber_df <- econ_situation_nber_df %>%
  mutate(NBER_recession = ifelse(date %in% NBER_recession_months, "Y", "N"),
         NBER_recession_next = as.factor(lead(NBER_recession,2))) %>%
  as.data.frame() %>%
  drop_na()
...
set.seed(2507)
# creating test-train data
testIDs <- createDataPartition(econ_situation_nber_df$NBER_recession_next, p = 0.6, list = F)
train_x <- econ_situation_nber_df[testIDs,c(6,7,8)]
train_y <- econ_situation_nber_df$NBER_recession_next[testIDs]
test_x <- econ_situation_nber_df[-testIDs, c(6,7,8)]
...
metric = "Accuracy"
# random forest model 
rf_recession <- train(x = test_x, y = test_y, method = "rf", metric = metric, trControl = trctrl, ntree = ntrees)
predict_y <- predict(rf_recession, newdata = test_x)
confusionMatrix(predict_y, reference = test_y)
