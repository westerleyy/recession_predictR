...
train_x <- econ_situation_nber_df[testIDs,c(4:15)]
train_y <- econ_situation_nber_df$NBER_recession_next[testIDs]
test_x <- econ_situation_nber_df[-testIDs, c(4:15)]
...
#bootstrapIDs <- createResample(training$NBER_recession_next, list = F)
...
gdp_complete <- gdp %>%
  mutate(date = as.yearmon(substr((Date), 1, 7)),
         ...
         `GDP (YoY % Change)` = `GDP (YoY % Change)`*100) %>%
  drop_na() %>%
  select(date, `GDP (YoY % Change)`, `GDP (Continuously Compounding)`, `Probability of Losing a Job`)
...
# model tuning 
trctrl <- trainControl(method = "repeatedcv", 
                       number = 10, repeats = 10, search = "grid")
mtry <- ncol(train_x)
ntrees <- 71
tunegrid <- expand.grid(.mtry = c(2:mtry))
metric = "Accuracy"
...
registerDoParallel(cl)
# gbm model 
gbm_recession <- train(NBER_recession_next~., data = training_bootstrapped,  method = "gbm", trControl = trctrl)
plot(gbm_recession)
stopCluster(cl)
gbm_recession
predict_y_prob <- predict(gbm_recession, newdata = testing, type = "prob")
